...
<body>
	<!-- 
		...
		Client     ->         Server(요청된 정보를 read(request객체))
			  (request 요청)
		...
		java코드로 (재요청)
		response.sendRedirect("Ex01_Basic.jsp") -> 서버에서 변경(client가 인지하는 코드로)
		>> 클라이언트 브라우져에서는 >> location.href = "Ex01_Basic.jsp"
	 -->
	
	1. 일반적인 출력() : <%=100+200+300 %>
	2. sendRedirect : response 객체의 함수
	<%
		//response.sendRedirect("Ex01_Basic.jsp"); // 페이지의 재요청 결과 동일
	%>
</body>
